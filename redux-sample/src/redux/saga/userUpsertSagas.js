...
function* UpsertUsers({ user }) {
  ...
  try {
    ...
    if (user.id === "") {
      ...
      yield put({ type: actionTypes.Complete_Add_USER_ACTION, value: user });
    } else {
      yield call(Api.ManageUser, "https://reqres.in/api/users", "PUT", user);

      yield put({ type: actionTypes.UPDATE_USER_ACTION, value: user });
    }

    yield put({ type: actionTypes.END_API_CALL, value: "success" });
  } catch (ex) {
    yield put({ type: actionTypes.USER_ERROR, value: ex });
